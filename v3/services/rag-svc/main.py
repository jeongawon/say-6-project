...
async def lifespan(app: FastAPI):
    """startup: FAISS 인덱스 + FastEmbed 로딩."""
    logger.info("rag-svc 시작 — FAISS 인덱스 + FastEmbed 로딩 중...")
    logger.info("model_dir=%s, embedder=%s", settings.model_dir, settings.embedder_model)
    try:
        # FAISS 인덱스 파일 + 메타데이터 + FastEmbed 모델 로드
        rag_service.load()
