...
FastEmbed(bge-small-en-v1.5, ONNX Runtime)로 쿼리 임베딩.

[핵심 흐름]
1. load()          → 서버 시작 시 FAISS 인덱스 + FastEmbed 로드
...
class RAGService:
    """FAISS 인덱스 + FastEmbed(ONNX) 기반 유사 케이스 검색."""
    ...
    def __init__(self):
        ...
        # FastEmbed 임베딩 모델 인스턴스 (ONNX Runtime 기반, PyTorch 불필요)
    ...
    def load(self) -> None:
        ...
        self._load_embedder()  # FastEmbed 모델 로드
        self._ready = True
    ...
    def _load_embedder(self) -> None:
        """FastEmbed 임베더 로드 (ONNX Runtime 기반, PyTorch 불필요)."""
    ...
    def _embed_query(self, text: str) -> np.ndarray:
        """FastEmbed로 쿼리 텍스트를 벡터로 변환 (ONNX Runtime)."""
        # embed()는 제너레이터 반환 → list()로 변환 후 첫 번째 요소
