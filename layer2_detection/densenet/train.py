"""
DenseNet-121 14-Disease Multi-label Classification — 올인원 SageMaker 스크립트

한 번 제출하고 자면 됨. Training Job이 알아서:
1. 소스 CSV 3종 로드 (say1-pre-project-5에서)
2. 전체 PA Master CSV 생성
3. 이미지 S3 선택 다운로드 (PA만, ~28GB)
4. 2-Stage Fine-tuning 학습
...
def build_full_pa_csv(image_bucket, work_bucket):
    ...
    # chexpert 라벨 병합 (inner join — 라벨 없는 study 제외)
    ...
    # image_path (로컬 상대경로 — S3 다운로드 시 prefix 붙임)
...
def save_checkpoint(state, checkpoint_dir):
    ...
    os.replace(tmp_path, path)  # 원자적 교체 — 쓰기 실패 시 기존 파일 보존
