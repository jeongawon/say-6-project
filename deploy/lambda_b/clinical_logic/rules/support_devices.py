...
# 대략적 px→cm 환산
PX_TO_CM_APPROX = 0.014  # ~0.14mm/px → 0.014cm/px
...
def analyze(input: ClinicalLogicInput) -> dict:
    ...
    for det in yolo_devices:
        ...
        if device == "ETT":
            # ETT 팁~carina 거리 판정
            # carina 위치 ≈ 두 폐 마스크 분기점 ≈ 이미지 상단 1/3
            tip_y = bbox[3]  # bbox 하단 = 팁 위치
            img_h = a.thorax_width_px if a.thorax_width_px > 0 else 512
            carina_y = img_h * 0.30  # 대략적 carina 위치

            dist_px = abs(tip_y - carina_y)
            dist_cm = round(dist_px * PX_TO_CM_APPROX, 1)
